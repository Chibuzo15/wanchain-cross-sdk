...
try {
  ...
  console.log(
    "Cardano library not available in nami.js - Cardano functionality will be disabled"
  );
...
class Nami {
  ...
  async getAccounts(network) {
    try {
      ...
      accounts = accounts.map((v) =>
        wasm.Address.from_bytes(Buffer.from(v, "hex")).to_bech32()
      );
      return accounts;
    } catch (err) {
  ...
  async sendTransaction(tx, sender) {
    let witnessSet = await this.cardano.signTx(
      Buffer.from(tx.to_bytes(), "hex").toString("hex")
    );
    witnessSet = wasm.TransactionWitnessSet.from_bytes(
      Buffer.from(witnessSet, "hex")
    );
    let transaction = wasm.Transaction.new(
      tx.body(),
      witnessSet,
      tx.auxiliary_data()
    );
    let txHash = await this.cardano.submitTx(
  ...
  async buildTx(
    ...
    txBuilder.add_change_if_needed(wasm.Address.from_bech32(paymentAddr));
