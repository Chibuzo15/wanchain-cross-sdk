...
try {
  ...
  console.log(
    "Cardano library not available in processAdaMintFromCardano - Cardano functionality will be disabled"
  );
...
module.exports = class ProcessAdaMintFromCardano {
  constructor(frameworkService) {
    ...
    this.m_iwanBCConnector = frameworkService.getService(
  ...
  async process(stepData, wallet) {
    ...
    try {
      let storemanGroupAddr =
        "addr_test1qz3ga6xtwkxn2aevf8jv0ygpq3cpseen68mcuz2fqe3lu0s9ag8xf2vwvdxtt6su2pn6h7rlnnnsqweavyqgd2ru3l3q09lq9e"; // smg address is manual specified
      console.debug(
        "ProcessAdaMintFromCardano storemanGroupAddr: %s",
        storemanGroupAddr
      );
      ...
      let storemanService =
        this.m_frameworkService.getService("StoremanService");
      ...
      let metaData = await this.buildUserLockData(
        params.tokenPairID,
        params.userAccount,
        params.fee
      );
      ...
      try {
        ...
        if (err === "Insufficient input in transaction") {
          webStores["crossChainTaskSteps"].finishTaskStep(
            params.ccTaskId,
            stepData.stepIndex,
            "",
            "Failed",
      ...
      try {
        ...
        webStores["crossChainTaskSteps"].finishTaskStep(
          ...
          txHash,
          ""
        ); // only update txHash, no result
      } catch (err) {
        console.error(
          "ProcessAdaMintFromCardano sendTransaction error: %O",
          err
        );
        if (err.code === 2) {
          // info: "User declined to sign the transaction."
          webStores["crossChainTaskSteps"].finishTaskStep(
            params.ccTaskId,
            stepData.stepIndex,
            "",
      ...
      let checkAdaTxService =
        this.m_frameworkService.getService("CheckAdaTxService");
  ...
  async initTx() {
    ...
    let p = await this.m_iwanBCConnector.getEpochParameters("ADA", {
      epochID: "latest",
    });
    let result = {
      linearFee: {
  ...
  minAdaRequired(value, minUtxo) {
    return wasm.min_ada_required(value, minUtxo).to_str();
  }
  ...
  buildUserLockData(tokenPairID, toAccount, fee) {
    ...
    if (tokenPairID !== NaN && toAccount.length === ToAccountLen) {
      let data = {
        5718350: {
          // define a special label, which is SLIP-0044 Wanchain Coin type
      ...
      data = wasm.encode_json_str_to_metadatum(
        JSON.stringify(data),
      ...
      if (ma) {
        ...
        for (let i = 0; i < scripts.len(); i++) {
          ...
          for (let j = 0; j < names.len(); j++) {
            ...
            tokens.push({
              script: tool.bytes2Hex(script.to_bytes()),
              name: name.name().toString(),
              value: assets.get(name).to_str(),
            });
          }
        }
      }
      outs.push({
        to: o.address().to_bech32(),
        coin: o.amount().coin().to_str(),
        tokens,
      });
    });
