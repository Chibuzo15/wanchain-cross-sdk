...
class TokenPairService {
  ...
  async init(frameworkService) {
    try {
      ...
      this.iwanBCConnector = frameworkService.getService(
      ...
      this.eventService.addEventListener(
        "iwanConnected",
        this.onIwanConnected.bind(this)
      );
      let tokenPairCfg = await this.configService.getGlobalConfig(
        "tokenPairCfg"
      );
      tokenPairCfg.map((tp) => {
    ...
    try {
      ...
      tokenPairs = tokenPairs.filter((tp) => {
        if (tp.ancestorSymbol !== "EOS") {
          ...
          if (this.updateTokenPairInfo(tp)) {
            // ignore unsupported token pair
      ...
      this.webStores.assetPairs.setAssetPairs(
        Array.from(tokenPairMap.values()),
        smgList
      );
  ...
  async readTokenpairs(startTime) {
    ...
    if (tokenPairs.length) {
      // maybe localstoreage TokenPair is cleared
      console.debug("all tokenpair hit cache");
    } else {
      let network = this.configService.getNetwork();
      let options =
        network === "mainnet" && !this.isTestMode
          ? { tags: ["bridge"] }
          : { isAllTokenPairs: true };
  ...
  async readMultiChainOrigToken(startTime) {
    let origTokens =
      await this.iwanBCConnector.getRegisteredMultiChainOrigToken();
    let map = new Map();
    ...
    let ts = new Date().getTime();
    console.debug(
      "readMultiChainOrigToken %d consume %s ms",
      origTokens.length,
      ts - startTime
    );
  }
  ...
  async readAssetLogos(tokenPairs, startTime) {
    ...
    tokenPairs.forEach((tp) => {
      if (tp.fromChainID === tp.ancestorChainID) {
        assetMap.set(tp.ancestorSymbol, {
          chain: tp.fromChainType,
          address: tp.fromAccount,
        });
      }
    });
    let cache = this.forceRefresh
      ? []
      : this.storageService.getCacheData("AssetLogo") || [];
    ...
    if (tokenScAddr.length) {
      let logos = await this.iwanBCConnector.getRegisteredTokenLogo({
        tokenScAddr,
        isAllTokenTypes: true,
      });
      // console.debug({logos});
      logos.forEach((v) => {
  ...
  async readChainLogos(tokenPairs, startTime) {
    ...
    let cache = this.forceRefresh
      ? []
      : this.storageService.getCacheData("ChainLogo") || [];
    let logoMapCacheOld = new Map(cache);
    let logoMapCacheNew = new Map();
    chainSet.forEach((k) => {
      let logo = logoMapCacheOld.get(k);
    ...
    if (newChains.length) {
      ...
      if (newChains.length * 3 > chainSet.size) {
        ...
        await Promise.all(
          newChains.map(async (chainType) => {
            let result = await this.iwanBCConnector.getRegisteredChainLogo({
              chainType,
            });
      ...
      logos.forEach((v) => {
        if (chainSet.has(v.chainType)) {
          logoMapCacheNew.set(v.chainType, {
            data: v.iconData,
            type: v.iconType,
          });
        }
  ...
  updateTokenPairInfo(tokenPair) {
    tokenPair.fromScInfo = this.chainInfoService.getChainInfoById(
      tokenPair.fromChainID
    );
    tokenPair.toScInfo = this.chainInfoService.getChainInfoById(
      tokenPair.toChainID
    );
    if (tokenPair.fromScInfo && tokenPair.toScInfo) {
      ...
      tokenPair.decimals =
        Number(tokenPair.fromDecimals) < Number(tokenPair.toDecimals)
          ? tokenPair.fromDecimals
          : tokenPair.toDecimals;
      try {
        ...
        console.error(
          "ignore unavailable token pair %s(%s, %s<->%s): %O",
          tokenPair.id,
          tokenPair.ancestorSymbol,
          tokenPair.fromChainName,
          tokenPair.toChainName,
          err
        );
  ...
  updateTokenPairCcHandle(tokenPair) {
    ...
    if (fromChainInfo.mintFromChainHandle) {
      ...
      if (tokenPair.fromChainID === tokenPair.ancestorChainID) {
        if (
          tokenPair.fromAccount === "0x0000000000000000000000000000000000000000"
        ) {
  ...
  getTokenBurnHandler(tokenPair, direction) {
    let chainType =
      direction === "MINT" ? tokenPair.fromChainType : tokenPair.toChainType;
    let tokenAccount =
      direction === "MINT" ? tokenPair.fromAccount : tokenPair.toAccount;
    let key = chainType + "-" + tokenAccount;
    let origToken = this.multiChainOrigToken.get(key);
    if (origToken) {
      if (direction === "BURN") {
        // console.debug(
        //   "tokenpair %s %s(%s<-%s) handler is MintErc20",
        //   tokenPair.id,
        //   origToken.symbol,
        //   tokenPair.fromChainType,
        //   tokenPair.toChainType
        // );
      }
      return "MintErc20";
    } else {
  ...
  getTokenEventType(tokenPairId, direction) {
    ...
    let chainType =
      direction === "MINT" ? tokenPair.toChainType : tokenPair.fromChainType;
    let tokenAccount =
      direction === "MINT" ? tokenPair.toAccount : tokenPair.fromAccount;
    let key = chainType + "-" + tokenAccount;
    let origToken = this.multiChainOrigToken.get(key);
    if (origToken || tokenAccount === tokenPair.ancestorAccount) {
      // original token or coin
      return "BURN"; // release
    } else {
      return "MINT";
    }
