...
class IWanBCConnector {
  ...
  async getMultiTokenInfo(chain, tokenScAddrArray, tokenType) {
    let options = tokenType ? { tokenType } : undefined;
    let ret = await this.apiClient.getMultiTokenInfo(
      chain,
      tokenScAddrArray,
      options
    );
  ...
  async getTokenBalance(chain, accountAddr, tokenAddr) {
    let ret = await this.apiClient.getTokenBalance(
      chain,
      accountAddr,
      tokenAddr
    );
  ...
  async getScEvent(chainType, address, topics, option) {
    let ret = await this.apiClient.getScEvent(
      chainType,
      address,
      topics,
      option
  ...
  async getStoremanGroupQuota(chainType, groupId, symbol, targetChainType) {
    let ret = await this.apiClient.getStoremanGroupQuota(
      chainType,
      groupId,
      symbol,
      targetChainType
  ...
  async getMinCrossChainAmount(targetChainType, symbol) {
    let ret = await this.apiClient.getMinCrossChainAmount(targetChainType, [
      symbol,
    ]);
    return ret;
  }
  ...
  async estimateNetworkFee(chainType, feeType, toChainType) {
    let ret = await this.apiClient.estimateNetworkFee(
      chainType,
      feeType,
      toChainType
    );
  ...
  async checkErc721Approved(chain, token, id, owner, operator) {
    ...
    return (
      isApprovedForAll || getApproved.toLowerCase() === operator.toLowerCase()
    );
  }

  async checkErc721Ownership(chain, token, id, address) {
    let abi = this.configService.getAbi("erc721");
    let owner = await this.apiClient.callScFunc(
      chain,
      token,
      "ownerOf",
      [id],
  ...
  async getNftInfoMulticall(
    ...
    for (let i = startIndex; i <= endIndex; i++) {
      result[i] = {
        id: ids.results.transformed[i]._hex,
        uri: uris.results.transformed[i],
      };
    }
    return result;
  }
  ...
  async estimateCrossChainOperationFee(chainType, targetChainType, options) {
    return this.apiClient.estimateCrossChainOperationFee(
      chainType,
      targetChainType,
      options
    );
  ...
  async estimateCrossChainNetworkFee(chainType, targetChainType, options) {
    return this.apiClient.estimateCrossChainNetworkFee(
      chainType,
      targetChainType,
      options
    );
